...
public class WebDriverFactory {
	 ...
	 */
	public WebDriver getInstance(ParamsUtils parameters, String downloadsFolder)
		...
		if (CHROME.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {
			...
			if (parameters.getPropValue(ParamsUtils.HEADLESS_MODE).equals("true"))
				options.addArguments("headless");


			if (parameters.getPropValue(ParamsUtils.URL).equals("")) {
				webDriver = new ChromeDriver(options);
			} else {

				if (parameters.getPropValue(ParamsUtils.PLATFORM_NAME).equals("Linux")) {
					options.addArguments("--no-sandbox");
					options.addArguments("--disable-dev-shm-usage");
				}
...
//						parameters.getPropValue(ParamsUtils.PLATFORM_NAME));
//				capabilities.setCapability("platformVersion", parameters.getPropValue(ParamsUtils.PLATFORM_VERSION));
//				capabilities.setCapability("browserVersion",
//						parameters.getPropValue(ParamsUtils.BROWSER_VERSION));

				webDriver = new RemoteWebDriver(new URL(parameters.getPropValue(ParamsUtils.URL)), capabilities);
			}

		} else if (FIREFOX.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {
			...
			webDriver = new FirefoxDriver(options);
				...
				DesiredCapabilities capabilities = new DesiredCapabilities();
				capabilities.setCapability(ChromeOptions.CAPABILITY, options);
//				capabilities.setCapability("platformName", parameters.getPropValue(ParamsUtils.PLATFORM_NAME));
//				capabilities.setCapability("platformVersion", parameters.getPropValue(ParamsUtils.PLATFORM_VERSION));
//				capabilities.setCapability("browserVersion", parameters.getPropValue(ParamsUtils.BROWSER_VERSION));
		...
		} else if (OPERA.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {

			System.setProperty("webdriver.opera.driver", "Profile\\Windows\\operadriver.exe");
			webDriver = new OperaDriver();

		} else if (INTERNET_EXPLORER.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {
			webDriver = new InternetExplorerDriver();

		} else if (EDGE.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {
			webDriver = new EdgeDriver();
		}

		else if (SAFARI.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {
			webDriver = new SafariDriver();

		}

		else if (MOBILE.equals(parameters.getPropValue(ParamsUtils.BROWSER_NAME))) {

			DesiredCapabilities capabilities = new DesiredCapabilities();
			capabilities.setCapability("deviceName", parameters.getPropValue(ParamsUtils.DEVICE_NAME));
			capabilities.setCapability(CapabilityType.VERSION, parameters.getPropValue(ParamsUtils.VERSION));// OS
																												// version
																												// of a
																												// device
			capabilities.setCapability("platformName", parameters.getPropValue(ParamsUtils.PLATFORM_NAME));
			capabilities.setCapability("appPackage", parameters.getPropValue(ParamsUtils.APP_PACKAGE));
			capabilities.setCapability("appActivity", parameters.getPropValue(ParamsUtils.APP_ACTIVITY));

			webDriver = new AppiumDriver<MobileElement>(new URL(parameters.getPropValue(ParamsUtils.URL)),
					capabilities);
		...
		if (webDriver != null) {
			try {
				try {
					...
					Logger.getLogger("SeleniumLog").error(
							"Please ignore. Seems as you use mobile web driver. In mobile driver.close() is not supported. ");
